 ...
 */
class RenderForm extends ScopedElementsMixin(DBPJobportalLitElement) {
    constructor() {
        super();
    ...
    connectedCallback() {
        ...
        window.addEventListener('DbpJobportalFormReset', this._onFormReset);
    ...
    disconnectedCallback() {
        ...
        window.removeEventListener('DbpJobportalFormReset', this._onFormReset);
    }
    ...
    async getUserAllSubmissionsData(formIdentifier) {
        ...
        try {
            ...
            const response = await this.httpGetAsync(
                this.entryPointUrl +
                    `/jobportal/submissions?formIdentifier=${formIdentifier}&perPage=100000&creatorIdEquals=` +
                    this.auth['user-id'],
                options,
            );
    ...
    async getSubmissionData() {
        ...
        try {
            const response = await this.httpGetAsync(
                this.entryPointUrl + '/jobportal/submissions/' + this.submissionId,
                options,
            );
    ...
    getFormHtml() {
        ...
        const tagName = 'dbp-jobportal-form-' + tagPart;
        ...
        if (Object.keys(this.loadedSubmission).length > 0) {
            ...
            if (
                ...
                `/jobportal/forms/${this.formIdentifiers[formUrlSlug]}`
            ) {
