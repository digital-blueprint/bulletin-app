...
/**
 * @augments {DBPJobportalLitElement}
 */
class ShowSubmissions extends ScopedElementsMixin(DBPJobportalLitElement) {
    constructor() {
        super();
        this.allForms = [];
                ...
                if (columnDefinition.field === 'htmlButtons') {
                    ...
                    columnDefinition.titleFormatter = (cell, formatterParams, onRendered) => {
                        let columnSettingsButton = this.submissionTables[state].createScopedElement(
                            'dbp-jobportal-column-settings-button',
                        );
    ...
    async getListOfAllForms() {
        ...
        try {
            ...
            const response = await fetch(
                this.entryPointUrl + '/jobportal/forms' + '?perPage=9999',
                {
                    headers: {
            ...
            if (!response.ok) {
                ...
                for (let x = 0; x < data['hydra:member'].length; x++) {
                    ...
                    let btn = this.formsTable.createScopedElement(
                        'dbp-jobportal-get-details-button',
                    );
    ...
    async getAllFormSubmissions(formId) {
        ...
        try {
            response = await this.httpGetAsync(
                this.entryPointUrl +
                    '/jobportal/submissions?formIdentifier=' +
                    formId +
                    '&perPage=9999',
                options,
            );
            ...
            for (let [x, submission] of submissions[state].entries()) {
                ...
                if (
                    ...
                    const submissionDetailsFormButton = this.submissionTables[
                        state
                    ].createScopedElement('dbp-jobportal-get-submission-link');
                    // Set submission URL
                    ...
                    if (columnDefinition.field === 'htmlButtons') {
                        ...
                        columnDefinition.titleFormatter = (cell, formatterParams, onRendered) => {
                            let columnSettingsButton = this.submissionTables[
                                state
                            ].createScopedElement('dbp-jobportal-column-settings-button');
                            columnSettingsButton.setAttribute('subscribe', 'lang');
                            columnSettingsButton.addEventListener('click', () => {
                                this.openColumnOptionsModal(state);
                            });
    ...
    async getAttachmentFilesDetails(submissionId) {
        ...
        try {
            const response = await this.httpGetAsync(
                this.entryPointUrl + '/jobportal/submissions/' + submissionId,
                options,
            );
            if (!response.ok) {
                this.handleErrorResponse(response);
    ...
    restoreSubmissionTableSettings(state) {
        if (this.storeSession && this.isLoggedIn()) {
            let optionsString = localStorage.getItem(
                `dbp-jobportal-tableoptions-${this.activeFormName}-${state}-${this.auth['user-id']}`,
            );
    ...
    storeSubmissionTableSettings(state) {
        if (this.storeSession && this.isLoggedIn()) {
            ...
            localStorage.setItem(
                `dbp-jobportal-tableoptions-${this.activeFormName}-${state}-${publicId}`,
    ...
    async apiUpdateSubmissionTags(submissionId, finalTags) {
        ...
        formData.append('form', '/jobportal/forms/' + this.activeFormId);
        ...
        const url = this.entryPointUrl + '/jobportal/submissions/' + submissionId;
    ...
    async apiGetTags(identifier) {
        ...
        try {
            response = await this.httpGetAsync(
                this.entryPointUrl + '/jobportal/forms/' + identifier,
                options,
            );
    ...
    render() {
        ...
        return html`
            ...
            <dbp-grant-permission-dialog
                ...
                resource-class-identifier="DbpRelayJobportalSubmission"></dbp-grant-permission-dialog>
            ...
            <dbp-modal
                id="deletion-confirmation-modal--jobportal"
